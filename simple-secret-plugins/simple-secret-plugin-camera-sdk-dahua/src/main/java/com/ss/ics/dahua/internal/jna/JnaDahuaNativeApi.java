 ...
 * @since 2026-08-12
 */
public final class JnaDahuaNativeApi implements DahuaNativeApi {
    ...
    private final DahuaNetSdkLibrary library;
    ...
    private final DahuaNetSdkLibrary.DisconnectCallback disconnectCallback =
            (loginId, ip, port, user) -> { };
    private final DahuaNetSdkLibrary.ReconnectCallback reconnectCallback =
            (loginId, ip, port, user) -> { };

    private JnaDahuaNativeApi(DahuaSdkOptions options, DahuaNetSdkLibrary library) {
     ...
     */
    public static JnaDahuaNativeApi load(DahuaSdkOptions options) {
        return load(options, System.getProperty("os.name"),
                path -> Native.load(path.toString(), DahuaNetSdkLibrary.class));
    }

    static JnaDahuaNativeApi load(
            DahuaSdkOptions options,
            String osName,
            Function<Path, DahuaNetSdkLibrary> loader) {
        if (loader == null) {
    ...
    public long startPreview(
        ...
        DahuaNetSdkLibrary.DataCallback nativeCallback = (handle, info, user) -> {
            try {
                if (!gate.enter()) {
                    return 1;
                }
    ...
    public long attachRadiometry(
        ...
        DahuaCallbackGate gate = new DahuaCallbackGate();
        DahuaNetSdkLibrary.RadiometryCallback nativeCallback =
