...
import static com.ss.gb28181.GbAlarmSubscriptionTest.*;
import static com.ss.gb28181.GbPlaybackTest.receiveMethod;
import static org.junit.jupiter.api.Assertions.*;

@Timeout(15)
class GbAlarmSubscriptionLifecycleTest {
    final String password = UUID.randomUUID().toString();

    @Test void rejectsUnknownDialogWrongLocalTagAndWrongMimeBeforeAcceptingSameSequence() throws Exception {
        int port = freePort(); var delivered = new LinkedBlockingQueue<GbAlarmEvent>();
        try (var server = Gb28181Server.open(options(port).build(), id -> Optional.of(password), delivered::add);
            ...
            GbAlarmSubscriptionTest.notify(peer, new Wire(sub.start(), wrong, sub.body()), 1, "remote", "presence", "active", "<bad");
            ...
            GbAlarmSubscriptionTest.notify(peer, new Wire(sub.start(), wrong, sub.body()), 1, "remote", "presence", "active", "<bad");
            assertEquals(481, peer.receive().status());
            peer.send(notification(peer, sub, 1, "text/plain", alarm(TARGET, 9)));
            assertEquals(415, peer.receive().status());
            peer.send(notification(peer, sub, 1, "Application/MANSCDP+xml", alarm(TARGET, 10)));
            assertEquals(200, peer.receive().status()); assertEquals(10, delivered.poll(2, TimeUnit.SECONDS).alarm().sn());
    ...
    @Test void refusesFullCallbackQueueWithoutBindingEarlyTagOrAdvancingCseq() throws Exception {
        ...
        try (var server = Gb28181Server.open(options(port).maxPendingAlarms(1).build(), id -> Optional.of(password), event -> {
            ...
            GbAlarmSubscriptionTest.notify(peer, sub, 1, "wrong", "presence", "active", alarm(TARGET, 2)); assertEquals(503, peer.receive().status());
            release.countDown(); await(() -> pendingAlarms(server) == 0);
            GbAlarmSubscriptionTest.notify(peer, sub, 1, "remote", "presence", "active", alarm(TARGET, 3)); assertEquals(200, peer.receive().status());
            respond(peer, sub, 200, "Expires: 60\r\n", "remote"); opening.get(2, TimeUnit.SECONDS);
    ...
    @Test void missingInitialNotifyFailsAtTimerNAndCleanupExpiryReleasesCapacity() throws Exception {
        int port = freePort();
        try (var server = Gb28181Server.open(options(port).maxAlarmSubscriptions(1).build(), id -> Optional.of(password), event -> { });
             var peer = new Peer(port, "UDP")) {
            peer.register(password); var opening = server.subscribeAlarms(DEVICE, TARGET, request()); var sub = peer.receive();
            ...
            var replacement = server.subscribeAlarms(DEVICE, TARGET, request()); var fresh = peer.receive();
            ...
            GbAlarmSubscriptionTest.notify(peer, sub, 1, "remote", "presence", "active", ""); assertEquals(481, peer.receive().status());
        }
    }
    ...
    @Test void oldTransactionTimeoutCannotFailAcceptedRenewal() throws Exception {
        ...
        try (var server = Gb28181Server.open(options(port).build(), id -> Optional.of(password), event -> { });
            ...
            GbAlarmSubscriptionTest.notify(peer, sub, 1, "remote", "presence", "active", ""); assertEquals(200, peer.receive().status());
            ...
            GbAlarmSubscriptionTest.notify(peer, sub, 2, "remote", "presence", "active", ""); assertEquals(200, peer.receive().status());
            var stopping = session.stop(); var stop = peer.receive(); respond(peer, stop, 200, "", "remote"); stopping.get(2, TimeUnit.SECONDS);
        }
    }

    @Test void ownerSubscriptionMayReportChildAndCallbackCanCloseServer() throws Exception {
        int port = freePort(); var delivered = new CountDownLatch(1); Gb28181Server[] holder = new Gb28181Server[1];
        try (var server = Gb28181Server.open(options(port).build(), id -> Optional.of(password), event -> {
            assertEquals(TARGET, event.alarm().deviceId()); holder[0].close(); delivered.countDown();
        }); var peer = new Peer(port, "UDP")) {
            holder[0] = server; peer.register(password); var opening = server.subscribeAlarms(DEVICE, DEVICE, request()); var sub = peer.receive();
            respond(peer, sub, 200, "Expires: 60\r\n", "remote"); var session = opening.get(2, TimeUnit.SECONDS);
            String wire = notification(peer, sub, 1, "Application/MANSCDP+xml", alarm(TARGET, 1))
    ...
    @Test void rebindContinuationCannotCreateSubscriptionAgainstRetiredBinding() throws Exception {
        ...
        try (var server = Gb28181Server.open(options(port).build(), id -> Optional.of(password), event -> { });
            ...
            session.completion().whenComplete((unused, failure) -> {
                try { server.subscribeAlarms(DEVICE, TARGET, request()); continuation.complete(true); }
            ...
            var fresh = server.subscribeAlarms(DEVICE, TARGET, request()); var next = rebound.receive();
    ...
    @Test void terminatedStateIgnoresExpiresZeroAndDeliversFinalAlarm() throws Exception {
        int port = freePort(); var delivered = new LinkedBlockingQueue<GbAlarmEvent>();
        try (var server = Gb28181Server.open(options(port).build(), id -> Optional.of(password), delivered::add);
             var peer = new Peer(port, "UDP")) {
            peer.register(password); var opening = server.subscribeAlarms(DEVICE, TARGET, request()); var sub = peer.receive();
            respond(peer, sub, 200, "Expires: 60\r\n", "remote"); var session = opening.get(2, TimeUnit.SECONDS);
            GbAlarmSubscriptionTest.notify(peer, sub, 1, "remote", "presence", "terminated;expires=0", alarm(TARGET, 1));
    ...
    static Object subscription(Gb28181Server server, Wire sub) {
        synchronized (server) {
            Object manager = field(server, "alarmSubscriptions");
    ...
    static int pendingAlarms(Gb28181Server server) { synchronized (server) { return (Integer) field(server, "pendingAlarms"); } }
    static void maintain(Gb28181Server server) throws Exception {
        Object manager = field(server, "alarmSubscriptions"); var method = manager.getClass().getDeclaredMethod("maintain");
