...
DJI SEI 诊断应用默认关闭所有原生媒体能力，`local` profile 才在 `0.0.0.0:7935` 启用匿名 RTMP 发布，
同时保持匿名播放、WebRTC 和管理 API 关闭。构建、`mk_api` 链接器前置条件、环境变量、结果判读与真实 DJI
验证边界见 [DJI RTMP SEI 诊断应用 README](simple-secret-application-dji-sei-test/README.md)。
