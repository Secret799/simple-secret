 ...
 *
 * @author JunPzx
 * @since 2025/8/20 18:23
 */
@Slf4j
public class VideoStackContext {
    ...
    public void init() {
        ...
        initStackWindow();
        initStatus.set(true);
        encodeFuture = SpringUtils.getSimpleSecretScheduledExecutor().submit(() -> {
            try {
                initEncodeAndPush();
            } finally {
    ...
    private void initFillImage() {
        ...
        int windowSize = param.getCol() * param.getRow();
        for (int i = 1; i <= windowSize; i++) {
            windowSet.add(i);
        }
        List<VideoStackWindowBO> urlList = param.getWindowList();
        if (urlList != null && !urlList.isEmpty()) {
            for (VideoStackWindowBO videoStackUrlParam : urlList) {
    ...
    private void initStackWindow() {
        List<VideoStackWindowBO> urlList = param.getWindowList();
        if (urlList != null && !urlList.isEmpty()) {
            for (VideoStackWindowBO VideoStackWindowBO : urlList) {
                int[] whp = calculateBlockDimensions(VideoStackWindowBO.getSpan(), param.getWidth(), param.getHeight(), param.getRow(), param.getCol(), linePointer.get(0));
                VideoStackWindowContext videoStackWindow = new VideoStackWindowContext(VideoStackWindowBO, param.getFillImgUrl(), whp[0], whp[1], whp[2], dataPointer, linePointer);
    ...
    private void initEncodeAndPush() {
        ...
        enCodecCtx.time_base(av_make_q(1, FPS));
        enCodecCtx.framerate(av_make_q(FPS, 1));
        ...
        if (isPush) {
            ...
            if (ret < 0) {
                avutil.av_log(enCodecCtx, AV_LOG_ERROR, "avformat_alloc_output_context2 error");
                free();
                return;
            }
            ...
            if (ret < 0) {
                avutil.av_log(enCodecCtx, AV_LOG_ERROR, "avcodec_parameters_from_context error");
                free();
                return;
            }
            avStream.codecpar().codec_tag(0);
        } else {
            MK_INI mkIni = ZlmMediaHelper.getZlmApi().mk_ini_create();
            ZlmMediaHelper.Configurator.setConfig(mkIni, "enable_rtsp", "1");
            ZlmMediaHelper.Configurator.setConfig(mkIni, "enable_rtmp", "1");
            ZlmMediaHelper.Configurator.setConfig(mkIni, "enable_hls", "1");
            ZlmMediaHelper.Configurator.setConfig(mkIni, "auto_close", 0);
            mkMedia = ZlmMediaHelper.getZlmApi().mk_media_create2(ZlmMediaServerConstants.DEFAULT_VHOST, param.getApp(), param.getId(), -1, mkIni);
            ZlmMediaHelper.getZlmApi().mk_ini_release(mkIni);
            ZlmMediaHelper.getZlmApi().mk_media_init_video(mkMedia, 0, param.getWidth(), param.getHeight(), FPS, (int) enCodecCtx.bit_rate());
            ZlmMediaHelper.getZlmApi().mk_media_init_complete(mkMedia);
        }
        ...
        if (avSwsCtx == null) {
            avutil.av_log(enCodecCtx, AV_LOG_ERROR, "sws_getContext error");
            free();
            return;
        }
        if (isPush) {
            ...
            while (true) {
                ...
                if (remaining > 0) {
                    ...
                    if (isPush) {
                        ...
                        ZlmMediaHelper.getZlmApi().mk_media_input_h264(mkMedia, new com.sun.jna.Pointer(avPacket.data().address()),
                                avPacket.size(), avPacket.pts(), avPacket.pts());
                    }
                } else {
                    if (isPush) {
                        avformat.av_interleaved_write_frame(oFmtCtx, avPacket);
                    } else {
                        ZlmMediaHelper.getZlmApi().mk_media_input_h264(mkMedia, new com.sun.jna.Pointer(avPacket.data().address()), avPacket.size(), avPacket.pts(), avPacket.pts());
                    }
                }
                lastPushTime = System.currentTimeMillis();
     ...
     * @param newParam
     */
    public void reset(VideoStackBO newParam) {
        if (!initStatus.get()) {
            throw new IllegalStateException("拼接屏任务尚未初始化成功，请稍后再试");
        }
        initStatus.set(false);
        for (VideoStackWindowContext videoStackWindow : windowList) {
    ...
    public void stop() {
        ...
        windowList.clear();
        Future<?> currentFuture = encodeFuture;
    ...
    private void free() {
        ...
        if (mkMedia != null) {
            ZlmMediaHelper.getZlmApi().mk_media_release(mkMedia);
            mkMedia = null;
        }
    }
        ...
        for (int currentRow = 1; currentRow < row; currentRow++) {
            ...
            for (int w = 0; w < lineWidth; w++) {
                ...
                for (int x = 0; x < width; x++) {
                    ...
                    data.put(pos, (byte) b);
                    // G
                    data.put(pos + 1, (byte) g);
                    // R
                    data.put(pos + 2, (byte) r);
                }
            }
        }
