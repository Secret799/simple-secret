...
class GbZlmPlaybackTest {
    ...
    void historyAllocatesBeforeInviteAndReleasesOnPlaybackEndOrOffline(boolean offline) throws Exception {
        var f = fixture();
    ...
    @Test
    void historyCancellationCancelsInviteAndReleasesReceiver() throws Exception {
        var f = fixture();
    ...
    @Test
    void liveAndHistoryShareReceiverCapacityAndCloseCancelsHistory() throws Exception {
        var f = fixture();
    ...
    @Test
    void historyAllocationFailureNeverInvitesAndInvalidArgumentsNeverAllocate() throws Exception {
        var f = fixture();
        try (GbZlmPlayer player = f.player(1, GbRtpTarget.Transport.UDP)) {
            assertThrows(NullPointerException.class, () -> player.playback(DEVICE, CHANNEL, null));
            assertThrows(IllegalArgumentException.class, () -> player.playback("bad", CHANNEL, RANGE));
            assertThrows(IllegalArgumentException.class, () -> player.playback(DEVICE, "bad", RANGE));
    ...
    @Test
    void rejectedHistoryInviteReleasesReceiverBeforeReturningFailure() throws Exception {
        var f = fixture();
        try (GbZlmPlayer player = f.player(1, GbRtpTarget.Transport.UDP)) {
            var opening = player.playback(DEVICE, CHANNEL, RANGE);
            assertTrue(f.invited.await(2, TimeUnit.SECONDS));
            IllegalStateException rejection = new IllegalStateException("history rejected");
    ...
    @Test
    void lateHistorySuccessAfterCloseStopsSipWithoutReopeningReceiver() throws Exception {
        var f = fixture();
        ...
        when(f.server.playback(eq(DEVICE), eq(CHANNEL), any(), eq(RANGE))).thenAnswer(call -> {
            f.invited.countDown();
    ...
    private static GbZlmPlayerTest.Fixture fixture() {
        ...
        when(f.server.playback(eq(DEVICE), eq(CHANNEL), any(), eq(RANGE))).thenAnswer(call -> {
            assertEquals(1, f.resources.size(), "RTP must exist before history INVITE");
