...
非法大小、空指针或空轨道只记录不含 payload 的有界参数化 WARN，不分配数组、不读取指针且不调用业务回调。

`mk_media_source_get_track` 返回复制引用。EasyMedia 在读取轨道元数据前接管每个有效 `MK_TRACK`，并由
原生媒体源指针对应的精确注册生命周期持有轨道和 JNA 回调。同指针重复注册复用现有生命周期，不重复通知或安装；
精确注销逐一调用一次 `mk_track_unref`，单个释放失败不阻断其他轨道，重复注销也不会重复释放。
处理器串行化低频的媒体源注册与注销控制事件，确保注销不会越过尚未完成的注册和轨道接管。
zlm4j 1.11.0 的 `mk_track_add_delegate` Java 绑定返回 `void`，无法取得删除所需 tag。
安装抛出异常时无法判断原生侧是否已经生效，因此实现会重新抛出异常，同时保留轨道和回调所有权到精确注销，
不会提前 unref 或伪造 tag 做显式删除。原生 delegate 由媒体源注销流程结束。
...
解析器按连续字节流识别三字节和四字节 Annex-B 起始码，因此一个片段可以包含多个 NALU，起始码也可以
跨片段边界。最后一个未遇到后续起始码的 NALU 会保留到下一片段继续解析。
如果 ZLM native frame 创建或输入失败，`pushWithBackpressure` 会向调用方抛出异常，不会把失败片段当作
已成功消费。
