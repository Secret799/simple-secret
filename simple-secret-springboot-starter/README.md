...
- `gb28181`：默认关闭的 GB28181 平台信令自动配置，包含报警复位、关键帧请求、报警上报与订阅、目录订阅、移动位置订阅、录像检索、实时点播、历史回放与下载信令，宿主提供设备凭据及可选报警/目录/位置监听器。
- `gb28181-zlm`：默认关闭的实时/历史/下载 RTP 接收适配，需显式配置 GB 信令与 ZLM。
